              ...
              <v-card-text>
                ...
                >
                  <template slot="items" scope="props">
                    ...
                    <td>
                      <v-btn small warning v-if="props.item.installed === true && props.item.restartable === true" :loading="props.item.loading"
                             @click="applyStateToComponent(props.item, 'restart')">
                        {{ $t('components.restart') }}
                      </v-btn>
                      <v-btn small primary v-if="props.item.installed === false && props.item.installable"
                             :loading="props.item.loading" @click="applyStateToComponent(props.item, 'install')">
                        {{ $t('components.install') }}
                      </v-btn>
                      <v-btn small error v-if="props.item.installed === true && props.item.installable"
                             :loading="props.item.loading" @click="applyStateToComponent(props.item, 'uninstall')">
                        {{ $t('components.uninstall') }}
                      </v-btn>

                      <v-btn small primary v-if="props.item.installed === true && props.item.enabled === false && props.item.toggleable"
                             :loading="props.item.loading" @click="applyStateToComponent(props.item, 'enabled')">
                        {{ $t('components.enable') }}
                      </v-btn>
                      <v-btn small error v-if="props.item.installed === true && props.item.enabled === true && props.item.toggleable"
                    ...
                    <span v-if="props.item.logs != false">
												<v-btn icon v-tooltip:right="{ html: $t('components.view_stdout_log') }"
                      ...
                      <span class="red--text" v-if="props.item.logs_error != false">
                               ...
                               v-tooltip:right="{ html: $t('components.view_stderr_log') }"

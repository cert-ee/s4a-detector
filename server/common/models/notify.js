...
module.exports = function (notify) {
  ...
  notify.initialize = function (cb) {
    ...
    (async () => {
      try {
        ...
        if (!notifications || notifications === undefined || notifications.lenght == 0 || notifications == "") {
		hell.o("count: " + notifications, "check notifications", "info")

		// Example alerts
		let input_data = [
		{
      name: "Diff aggregation 1 to root",
			query: '{ "size": 0, "query": { "bool": { "must": [ { "range": { "timestamp": { "gt": "now-30m" }} } ] } }, "aggs": { "notify": { "date_histogram": { "field": "@timestamp", "interval": "30m" }, "aggs": { "count_me": { "value_count": { "field": "@timestamp" } }, "diff": { "serial_diff": { "buckets_path": "count_me" } }, "treshold": { "bucket_selector": { "buckets_path": { "alarms_count": "count_me.value" }, "script": "params.alarms_count > 100" } } } } } }',
      ...
      name: "Count Aggregation to postmaster",
			query: '{ "size": 0, "query": { "bool": { "must": [ { "range": { "timestamp": { "gt": "now-60m" }} } ] } }, "aggs": { "notify": { "date_histogram": { "field": "@timestamp", "interval": "30m" }, "aggs": { "treshold": { "bucket_selector": { "buckets_path": { "alarms_count": "_count" }, "script": "params.alarms_count > 100" } } } } } }',
			email: "postmaster@localhost",
			subject: "Count Aggregation to postmaster",
                        enabled: true
		},
		{
      name: "Severity 1 to root",
			query: '{"query": { "bool": { "must": [ { "term": { "alert.severity": 1 } } ] } } }',
			email: "root@localhost",
			subject: "Severity 1 to root",
      enabled: false
		},
		{
      name: "Severity 1 to root",
        ...
        // TODO: check for host/port/auth - if resolves, port answers, auth succeeds
  ...
  notify.checkAlerts = function (input) {
	...
	if (!elastic_params.body && !elastic_params.body.size)
    ...
    (async function () {
      try {
		...
		if (alert_count_to_send == 0 && !aggregation_match) { //nothing to send
			hell.o("done, no alerts to notify, ptr: " + notifying.alerts_pointer , "notifyRoutine", "info");
			continue;
		}

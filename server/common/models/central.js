...
module.exports = function (central) {
   ...
   * @returns {Promise}
   */
  central.updateToken = function (input) {
    ...
    return new Promise((success, reject) => {
      (async () => {
        try {
          ...
          central.initialize(function () {
            hell.o("done", "updateToken", "info");
            success(central_result);
          });
        } catch (err) {
   ...
   * @param cb
   */
  central.initialize = function (cb) {
   ...
   * @param type_result
   */
  central.lastSeen = function (central_status, type, type_result) {
     ...
     */
    if (type !== undefined && ["alerts", "central", "components", "rules"].includes(type)) {
